      ...
      for (int i=il; i<=iu; ++i) {
        ...
        if (x < x_mid) {
          ...
          if (flag) {
            ...
            std::cout << "egas = " << prfld2->u_gas(k,j,i) << std::endl;
            std::cout << "Erad = " << prfld2->u_rad(k,j,i) << std::endl;
            flag = false;
          }
        } else {
...
void AddRadiativeForceAndWork(MeshBlock *pmb, const Real time, const Real dt,
  ...
  // if ((pmb->iuser_meshblock_data[TSTEP_COUNTER](0) + 1) % rk_cycle == 0) {
    ...
    int jl = pmb->js, ju = pmb->je;
    int kl = pmb->ks, ku = pmb->ke;
    Real idx1 = 1.0/pmb->pcoord->dx1f(pmb->is);
    Real idx2 = 0.0;
    Real idx3 = 0.0;
    if (pmb->block_size.nx2 > 1) idx2 = 1.0/pmb->pcoord->dx2f(pmb->js);
    if (pmb->block_size.nx3 > 1) idx3 = 1.0/pmb->pcoord->dx3f(pmb->ks);
    Real hidx1 = 0.5*idx1;
    Real hidx2 = 0.5*idx2;
    Real hidx3 = 0.5*idx3;
    ...
    for (int k = kl; k <= ku; ++k) {
      for (int j = jl; j <= ju; ++j) {
        for (int i = il; i <= iu; ++i) {
          Real lambda;
          Real dEr[3];
          dEr[0] = hidx1*(fld_u(k,j,i+1) - fld_u(k,j,i-1));
          dEr[1] = hidx2*(fld_u(k,j+1,i) - fld_u(k,j-1,i));
          dEr[2] = hidx3*(fld_u(k+1,j,i) - fld_u(k-1,j,i));
...
Real HistoryTg(MeshBlock *pmb, int iout) {
  ...
  for (int k=ks; k<=ke; k++) {
    for (int j=js; j<=je; j++) {
      for (int i=is; i<=ie; i++) {
        T += pmb->peos->TempFromRhoEg(pmb->phydro->w(IDN,k,j,i), pmb->prfld2->u_gas(k,j,i))*T_unit;
        num++;
      }
    }
  }
...
Real HistoryaTg4(MeshBlock *pmb, int iout) {
  ...
  for (int k=ks; k<=ke; k++) {
    for (int j=js; j<=je; j++) {
      for (int i=is; i<=ie; i++) {
        aT4 += std::pow(pmb->peos->TempFromRhoEg(pmb->phydro->w(IDN,k,j,i), pmb->prfld2->u_gas(k,j,i)), 4);
        num++;
      }
    }
  }
