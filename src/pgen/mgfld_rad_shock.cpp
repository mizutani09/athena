...
void FLDInnerX1(AthenaArray<Real> &dst, Real time, int nvar,
  ...
  for (int k=ks; k<=ke; k++) {
    for (int j=js; j<=je; j++) {
      for (int i=1; i<=ngh; i++) {
        dst(RadFLD2::GAS,k,j,is-i) = dst(RadFLD2::GAS,k,j,is+i-1);
        dst(RadFLD2::RAD,k,j,is-i) = dst(RadFLD2::RAD,k,j,is+i-1);
      }
    }
  }
  return;
...
//     for (int j=js; j<=je; j++) {
//       for (int i=1; i<=ngh; i++) {
//         dst(RadFLD2::GAS,k,j,ie+i) = egas0;
//         dst(RadFLD2::RAD,k,j,ie+i) = Er0;
//       }
//     }
//   }
//   return;
...
void FLDOuterOutflowX1(AthenaArray<Real> &dst, Real time, int nvar,
  ...
  for (int k=ks; k<=ke; k++) {
    for (int j=js; j<=je; j++) {
      for (int i=1; i<=ngh; i++) {
        dst(RadFLD2::GAS,k,j,ie+i) = dst(RadFLD2::GAS,k,j,ie);
        dst(RadFLD2::RAD,k,j,ie+i) = dst(RadFLD2::RAD,k,j,ie);
      }
    }
  }
  return;
  ...
  for(int k=kl; k<=ku; ++k) {
    for (int j=jl; j<=ju; ++j) {
      for (int i=il; i<=iu; ++i) {
        ...
        // for FLD
        prfld->u_gas(k,j,i) = egas0;
        prfld->u_rad(k,j,i) = Er0;
      }
    }
  }
  return;
  ...
  for (int k=kl; k<=ku; k++) {
    for (int j=jl; j<=ju; j++) {
      for (int i=il; i<=iu; i++) {
        ...
        // for Trad
        user_out_var(3,k,j,i) = std::pow(prfld->u_rad(k,j,i)/a_r_sim, 0.25)*T_unit;
      }
    }
  }
  return;
