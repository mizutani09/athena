...
FLD::FLD(MeshBlock *pmb, ParameterInput *pin) :
    ...
    output_defect(false), fldbvar(pmb, &ecr, &coarse_ecr, empty_flux, false), //!
    refinement_idx_(), Dpara_(), Dperp_(), Lambda_() {
  // Dpara_ = pin->GetReal("mgfld", "Dpara"); //!
  // Dperp_ = pin->GetReal("mgfld", "Dperp"); //!
  // Lambda_ = pin->GetReal("mgfld", "Lambda"); //!
  ...
  fldbvar.bvar_index = pmb->pbval->bvars.size();
  pmb->pbval->bvars.push_back(&fldbvar);
  pmb->pbval->pfldbvar = &fldbvar;
}
